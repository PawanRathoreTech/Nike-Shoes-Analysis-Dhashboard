# Nike-Shoes-Analysis-Dashboard-Power_Bi

## 📸 Dashboard Preview

### Overview
![Overview](https://github.com/PawanRathoreTech/Nike-Shoes-Analysis-Dhashboard/blob/main/Screenshot%202026-01-17%20231608.png)

### Insight
![Insight](https://github.com/PawanRathoreTech/Nike-Shoes-Analysis-Dhashboard/blob/main/Screenshot%202026-01-17%20231642.png)

### Performance
![Performance](https://github.com/PawanRathoreTech/Nike-Shoes-Analysis-Dhashboard/blob/main/Screenshot%202026-01-17%20231714.png)

### Overview 2
![Overview](https://github.com/PawanRathoreTech/Nike-Shoes-Analysis-Dhashboard/blob/main/Screenshot%202026-01-17%20231800.png)

### Insight 2
![Insight](https://github.com/PawanRathoreTech/Nike-Shoes-Analysis-Dhashboard/blob/main/Screenshot%202026-01-17%20231817.png)

### Performance 2
![Performance](https://github.com/PawanRathoreTech/Nike-Shoes-Analysis-Dhashboard/blob/main/Screenshot%202026-01-17%20231817.png)


## 📊 Project Overview
This project presents a detailed **sales analysis of Nike shoes** using an
interactive **Power BI dashboard**.  
The analysis focuses on **regional performance, monthly sales trends, product-level insights,
pricing, ratings, and profitability** to support data-driven business decisions.

## 🔧 Tools & Technologies Used
- **Power BI** – Dashboard design & data visualization  
- **SQL** – Data extraction, filtering, and transformation  
- **DAX** – Calculated measures & KPIs  
- **Excel / CSV** – Data source

- ## 📌 Dashboard Pages
The dashboard is divided into three main sections:

### 🔹 Overview
- Year-wise unit sales comparison (2020 & 2021)
- Monthly unit sales trend
- Region-wise unit sales distribution
- Overall profit analysis

### 🔹 Insight
- Product-level analysis (Air Jordan series)
- Sale price vs listing price comparison
- Customer reviews and ratings
- Individual product performance insights

### 🔹 Performance
- Regional sales performance comparison

- 
- Monthly sales fluctuations
- Profit contribution by region
- High-performing vs low-performing regions

- ## 📊 Key Metrics Highlighted
- Total Units Sold (Year-wise)
- Monthly Units Sale Trend
- Profit Analysis
- Sale Price & Listing Price
- Product Ratings & Reviews
- Region-wise Performance

---

## 🎯 Objective
To demonstrate **data analytics and visualization skills** by transforming raw sales data
into meaningful insights using **Power BI, SQL, and DAX**.

---

## 📂 Deliverables
- Interactive Power BI Dashboard
- Cleaned and modeled dataset
- Business-ready insights
- Visual storytelling for decision-making

---

## 👤 Author
**Pawan Rathore**  
Aspiring Data Analyst | Power BI | SQL | DAX  

---

### 🔗 Connect With Me

- LinkedIn: https://www.linkedin.com/in/pawan-rathore-b32b08383/
